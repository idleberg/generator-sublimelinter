...
licensesToTest.forEach((licenseName) => {
	const LicenseTest = suite(`with ${licenseName} license`);

	LicenseTest.before(async () => {
		await helper({
			name: 'test-linter',
			version: '0.1.0',
			interface: 'Linter',
	...
	LicenseTest('creates LICENSE file', () => {
		assert.file('LICENSE');
	});

	LicenseTest(`contains ${licenseName} license text`, () => {
		const licenseText = spdxLicenseList[licenseName].licenseText.replace(/\n{3,}/g, '\n\n');
		assert.fileContent('LICENSE', licenseText);
	});

	LicenseTest('mentions license in README', () => {
