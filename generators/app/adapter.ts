...
export class ClackCompatAdapter extends TerminalAdapter {
	...
	private async askQuestion(question: any, answers: any): Promise<any> {
		...
		switch (question.type) {
			...
			case 'expand': {
				// Filter out separators (objects without value/key or with type='separator')
				...
				// Default to first option if no default specified (common in file conflicts)
				const initialValue = defaultValue !== undefined ? defaultValue : expandOptions[0]?.value;

				const expandResult = await clack.multiselect({
